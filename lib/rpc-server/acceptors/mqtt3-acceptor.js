...
pro.listen = function(port) {
    ...
    this.server = net.CreateServer(aedes);
    this.server.listen(port, function (client, error) {
        console.log('server started and listening on port ', port)
    });

    this.server.on('clientError', function (client, err) {
        logger.error('rpc client error', client.id, err.message, err.stack)
    });

    this.server.on('connectionError', function (client, err) {
        logger.error('rpc client error', client, err.message, err.stack)
    })
    this.server.on('clientReady', function(client){
        self.sockets[client.id] = client;
    });
    this.server.on('publish', function(packet, client){
        if(client){
    ...
    this.server.on("clientDisconnect", function(client){
